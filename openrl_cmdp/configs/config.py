...
from openrl_cmdp.configs.utils import ProcessYamlAction
...
def create_config_parser():
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Names of cost signals exposed through env infos for CMDP objective. "
            "Leave empty to disable CMDP handling."
        ),
    )
    parser.add_argument(
        "--cmdp_constraint_budgets",
        type=float,
        nargs="*",
        default=None,
        help=(
            "Soft upper bounds for each CMDP cost stream. "
            "Length must match cmdp_cost_keys when provided."
        ),
    )
    parser.add_argument(
        "--cmdp_dual_step_size",
        type=float,
        default=0.01,
        help="Step size for CMDP dual ascent updates.",
    )
    parser.add_argument(
        "--cmdp_lambda_init",
        type=float,
        ...
        default=None,
        help=(
            "Initial values for CMDP Lagrange multipliers. "
            "Length must match cmdp_cost_keys when provided."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum clamp values for CMDP Lagrange multipliers. "
            "Length must match cmdp_cost_keys when provided."
        ),
    )
    parser.add_argument(
        "--cmdp_use_lambda_tanh_param",
        action="store_true",
        default=False,
        help="If set, use a tanh-reparameterized unconstrained variable for each CMDP lambda.",
    )
    parser.add_argument(
        "--cmdp_adv_norm_per_stream",
        action="store_true",
        default=True,
        help="Normalize reward/cost advantages per stream before forming the CMDP objective.",
    )
    parser.add_argument(
        "--cmdp_match_discount_for_cost",
        action="store_true",
        default=True,
        help="Use the same gamma discount for reward and all CMDP cost streams.",
    )
    parser.add_argument(
        "--cmdp_ema_momentum",
        type=float,
        default=0.9,
        help="Momentum for exponential moving average of CMDP cost estimates.",
    )
    # ppg parameters
    parser.add_argument(
        "--aux_epoch",
