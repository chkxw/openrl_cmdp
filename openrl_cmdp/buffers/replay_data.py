...
from openrl_cmdp.buffers.utils.obs_data import ObsData
from openrl_cmdp.buffers.utils.util import (
    _cast,
    _cast_v3,
    _flatten,
    _flatten_v3,
...
class ReplayData(object):
    def __init__(
        ...
        self.value_head_dim = 1 + self.cmdp_cost_count
        ...
        if self.cmdp_cost_count > 0:
            self.cmdp_costs = np.zeros(
                (
                    self.episode_length,
                    self.n_rollout_threads,
                    num_agents,
                    self.cmdp_cost_count,
                ),
                dtype=np.float32,
            )
        else:
            self.cmdp_costs = None
    ...
    def insert(
        ...
        if self.cmdp_cost_count > 0:
            if costs is None:
                self.cmdp_costs[self.step].fill(0.0)
            else:
                self.cmdp_costs[self.step] = costs.copy()
    ...
    def _compute_single_stream_returns(
        self,
        signal,
        next_value,
        value_preds_stream,
        ...
        """
        Compute discounted returns for a single reward/cost stream using the
        configured PPO settings (GAE, proper time limits, masks).
        """

        value_preds_stream[-1] = next_value

        use_normalizer = (
            (self._use_popart or self._use_valuenorm) and value_normalizer is not None
        )

        if self._use_proper_time_limits:
            if self._use_gae:
                gae = np.zeros_like(next_value)
                for step in reversed(range(signal.shape[0])):
                    mask = self.masks[step + 1]
                    bad_mask = self.bad_masks[step + 1]

                    if use_normalizer:
                        next_val = value_normalizer.denormalize(
                            value_preds_stream[step + 1]
                        )
                        curr_val = value_normalizer.denormalize(
                            value_preds_stream[step]
                        )
                    else:
                        next_val = value_preds_stream[step + 1]
                        curr_val = value_preds_stream[step]

                    delta = signal[step] + self.gamma * next_val * mask - curr_val
                    gae = delta + self.gamma * self.gae_lambda * mask * gae
                    gae = gae * bad_mask

                    target = (
                        value_normalizer.denormalize(value_preds_stream[step])
                        if use_normalizer
                        else value_preds_stream[step]
                    )
                    returns_stream[step] = gae + target
            else:
                returns_stream[-1] = next_value
                for step in reversed(range(signal.shape[0])):
                    mask = self.masks[step + 1]
                    bad_mask = self.bad_masks[step + 1]

                    next_return = returns_stream[step + 1]
                    ret = next_return * self.gamma * mask + signal[step]
                    corrected = (
                        value_normalizer.denormalize(value_preds_stream[step])
                        if use_normalizer
                        else value_preds_stream[step]
                    )
                    returns_stream[step] = ret * bad_mask + (1 - bad_mask) * corrected
        else:
            if self._use_gae:
                gae = np.zeros_like(next_value)
                for step in reversed(range(signal.shape[0])):
                    ...
                    delta = signal[step] + self.gamma * mask * next_val - curr_val
                    gae = delta + self.gamma * self.gae_lambda * mask * gae
                ...
                for step in reversed(range(signal.shape[0])):
                    mask = self.masks[step + 1]
                    returns_stream[step] = (
                        returns_stream[step + 1] * self.gamma * mask + signal[step]
                    )

    def compute_returns(self, next_value, value_normalizer=None):
        head_signals = [self.rewards]
        if self.cmdp_cost_count > 0:
            head_signals.extend(
                [self.cmdp_costs[..., idx : idx + 1] for idx in range(self.cmdp_cost_count)]
            )

        for head_idx, signal in enumerate(head_signals):
            next_value_stream = next_value[..., head_idx : head_idx + 1]
            value_preds_stream = self.value_preds[..., head_idx : head_idx + 1]
            returns_stream = self.returns[..., head_idx : head_idx + 1]
            stream_normalizer = value_normalizer if head_idx == 0 else None

            self._compute_single_stream_returns(
                signal,
                next_value_stream,
                value_preds_stream,
                returns_stream,
                value_normalizer=stream_normalizer,
            )
