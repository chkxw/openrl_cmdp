...
from openrl_cmdp.envs.nlp.utils.text_generation_pool import Sample
...
class Observation:
    ...
    def _concat(
        ...
        concatenated[:, full_size - actual_size :] = torch.cat(
            (prompt_, context_), dim=1
        )
    ...
    def update(self, action: int, tokenizer: AutoTokenizer) -> "Observation":
        ...
        token = tokenizer._convert_id_to_token(action)
        current_action_history.append(token)
        ...
        current_context_attention_mask[:, 0:-1] = current_context_attention_mask[
            :, 1:
        ].clone()
        ...
        input_encoded_pt, input_attention_mask_pt = Observation._concat(
            self.prompt_or_input_encoded_pt,
    ...
    def init_from_sample(
        ...
        if context_start_token is not None:
            ...
            context_outputs.input_ids = (
                torch.ones(1, max_context_length, dtype=torch.int32)
                * tokenizer.pad_token_id
            )
            context_outputs.input_ids[:, -1] = context_start_token
            context_outputs.attention_mask = torch.zeros(
                1, max_context_length, dtype=torch.int32
            )
        ...
        input_encoded_pt, input_attention_mask_pt = Observation._concat(
            ...
            context_outputs.input_ids,
            context_outputs.attention_mask,
            tokenizer.pad_token_id,
        )
