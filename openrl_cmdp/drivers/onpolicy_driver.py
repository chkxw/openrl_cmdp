...
from openrl_cmdp.drivers.rl_driver import RLDriver
...
class OnPolicyDriver(RLDriver):
    ...
    def add2buffer(self, data):
        ...
        if self.cmdp_cost_count > 0:
            cost_batch = np.zeros(
                (self.n_rollout_threads, self.num_agents, self.cmdp_cost_count),
                dtype=np.float32,
            )
            for env_idx, info in enumerate(infos):
                for cost_idx, key in enumerate(self.cmdp_cost_keys):
                    if key not in info:
                        continue
                    cost_val = info[key]
                    if isinstance(cost_val, dict):
                        for agent_idx, value in cost_val.items():
                            cost_batch[env_idx, agent_idx, cost_idx] = float(value)
                    else:
                        cost_batch[env_idx, :, cost_idx] = float(cost_val)
    ...
    def compute_returns(self):
        ...
        value_head_dim = 1 + self.cmdp_cost_count
        if next_values is None:
            next_values = np.zeros(
                [self.learner_n_rollout_threads, self.num_agents, value_head_dim]
            )
    ...
    def act(
        ...
        if value is None:
            values = np.zeros(
                [self.n_rollout_threads, self.num_agents, value_head_dim]
            )
        else:
